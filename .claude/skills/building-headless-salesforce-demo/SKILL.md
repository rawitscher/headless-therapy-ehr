...
- **Inspiration / competitor tools to emulate?** — the "we wish Salesforce looked like X" answer. Ask for 1–3 named products (and screenshots/links if they have them). This is the single biggest lever on visual quality: you'll mirror that tool's *layout density and component vocabulary* (stat rows, dense tables, charts, timelines), not just its colors. A demo that looks like a real vertical-SaaS product lands; a sparse "form on a page" does not.
- **Who's in the demo room?** — persona(s) + seniority (e.g., "VP of Ops + 2 frontline bankers" vs. "CIO + procurement"). This tunes density and tone: frontline personas want dense, operational, data-rich screens; executives want a cleaner, headline-driven view. Note it — it drives how much to pack onto the hero page in Phase 4 and what the Phase 4.5 grill-me checks against.

→ Open [industry-playbooks.md](industry-playbooks.md) and find the matching vertical section. Use it as your starting scaffold (hero surface, personas, **required visual components**, custom fields, AI insight cards, flow ideas, terminology). If the customer's vertical isn't listed, pick the closest one and adapt.
...
- **Deployment target** — confirm they have the prepared SDO from the Slack prework (the default). Only deviate if they explicitly want an existing dev org.
...
### Phase 2: Connect to the SDO (5–10 min)

**As of React Multi-Framework GA (June 2026), you do NOT create a scratch org and you do NOT flip Setup toggles.** The user arrives with a **Simple Demo Org (SDO)** that the Slackbot prework already prepared — it spun up the SDO and enabled the three prerequisites (Einstein, Agentforce, and the React/Multi-Framework App Domain). Your job here is only to **get authed into that SDO and confirm it's actually ready**, then move on.

> 💡 **What an SDO is, in one line:** a Simple Demo Org is a pre-provisioned, demo-ready Salesforce org (requested via the `@STORM` Slack app). Everything in this build deploys to that SDO. If the user says they don't have one or it's old, point them back to Slack (see "If the probe fails" below) — you do not provision orgs from the IDE.

**Step 2A — Authenticate into the SDO.**

Ask the user to log in via the CLI (this is the one manual thing you need from them here). Have them run:

```bash
sf org login web --alias <alias> --set-default
```

This opens a browser to the Salesforce login; they sign in with the SDO credentials from their Slack confirmation. When it returns, you're authed and `<alias>` is the default org for the rest of the build.
...
**Step 2B — Probe the SDO to confirm it's ready (don't assume the prework was done right).**

The prework *should* have handled everything, but a distracted user can skip a step or hand you an old org. Run two cheap checks and report the result plainly before building.

1. **Org age — must be created on/after June 15, 2026** (older SDOs don't have React/Multi-Framework available):

```bash
sf org display --target-org <alias> --json 2>/dev/null \
  | python3 -c "import sys,json; s=sys.stdin.read(); d=json.loads(s[s.find('{'):])['result']; print('Created:', d.get('createdDate'), '| Edition:', d.get('edition'), '| User:', d.get('username'))"
```

If `createdDate` is before 2026-06-15, the org is too old → send them back to Slack prework Step 1 (request a fresh SDO via `@STORM`).

2. **Prerequisites enabled — Einstein, Agentforce, and the React App Domain.** The reliable signal is whether the actual build steps work, so verify functionally rather than scraping Setup:
   - **React App Domain (most important):** the first UI-bundle deploy in Phase 3 is the real test. If it fails with *"UIBundle Metadata API is not enabled"* or a domain error, the React domain wasn't enabled → prework Step 4.
   - **Einstein + Agentforce:** confirmed when Phase 6 publishes/activates the agent. You can sanity-check earlier by opening the org (`sf org open -o <alias>`) and glancing at Setup → "Agentforce Agents", but don't burn time here — a failed agent publish in Phase 6 is the definitive signal → prework Steps 2–3.

**If the probe fails — bounce them back to Slack, don't try to fix it from the IDE.** Enabling Einstein/Agentforce/the React domain is **Slackbot prework**, not your job, and several of these can't be done from the CLI anyway. Tell the user exactly which prework step to redo, in their SDO's Setup:

| Missing | Symptom you'll hit | Send them to (prework step) |
|---|---|---|
| Recent SDO | `createdDate` < 2026-06-15 | Step 1 — request a new SDO from the `@STORM` Slack app |
| Einstein | Agent publish/activate fails in Phase 6 | Step 2 — Setup → search **Einstein** → Einstein Setup → toggle on → refresh |
| Agentforce | "Agentforce Agents" page empty / publish fails | Step 3 — Setup → search **Agentforce Agents** → toggle on |
| React App Domain | UI-bundle deploy fails ("UIBundle Metadata API is not enabled" / domain error) | Step 4 — Setup → search **Multi-Framework** → **Enable Domain** under "Enable the Salesforce App Domain" |

Surface the specific gap as the LAST block in your message and **fire a desktop notification** (see "Nudge the user"). Once they confirm the prework step is done, re-run the relevant check and continue.
...
#### The universal visual bar (non-negotiable minimum)

⚠️ **The #1 quality failure mode of this skill is a hero page that's too sparse** — a title, a few text fields, and a couple of cards. That reads as "a form," not "a product the customer would pay to keep." Every hero detail page you build MUST hit this minimum before you consider Phase 4 done. Build with the existing stack: **shadcn/ui + Tailwind + Recharts (charts/sparklines) + lucide-react (icons)**.

Universal minimum for the hero detail page (the playbook adds vertical-specific extras on top):

1. **KPI / stat-card row** — 3–5 metric cards across the top, each with a number, a label, and a trend indicator (up/down arrow + delta, or a sparkline). Not plain text.
2. **Rich entity header** — avatar or monogram, entity name, 2–3 status badges, the clickable Salesforce CRM link badge, and the ONE write-back action button (Phase 4 / [salesforce-action-button.md](salesforce-action-button.md)).
3. **Tabbed detail section — ≥4 tabs**, one of which is the **2×2 AI Insights grid**. Other tabs come from the playbook (e.g., Activity, Holdings, Cases, Billing).
4. **≥1 dense data table or list** — sortable-looking rows with badges, dates, and values. This is what sells "real product." A single table of 6–10 rows beats three paragraphs of text.
5. **≥1 chart / visualization** — a Recharts trend line, sparkline, gauge, or donut. Even one small chart dramatically raises perceived sophistication.

If any of these five is missing, the hero page isn't done. The per-vertical **Required visual components** checklist in [industry-playbooks.md](industry-playbooks.md) extends this with vertical-specific must-haves — hit both.
...
### Phase 4.5: Creative validation — "grill me" before you move on (15 min, DO NOT SKIP)

The hero surfaces are the demo. A sparse or off-brand hero page sinks the whole build, and it's far cheaper to fix now than after data, agent, and flow are wired on top. **Do not proceed to Phase 5 until the UI clears this gate.** Two steps: agent self-critique, then grill the user.

**Step 4.5A — Self-critique against the bar (do this BEFORE asking the user anything).**

Deploy the current build and look at it honestly. Score the hero page against both checklists and state the result plainly:

- Universal visual bar (Phase 4): KPI/stat row ✅/❌, rich entity header ✅/❌, ≥4 tabs incl. 2×2 AI grid ✅/❌, ≥1 dense data table ✅/❌, ≥1 chart ✅/❌.
- The vertical's **Required visual components** checklist in [industry-playbooks.md](industry-playbooks.md): each item ✅/❌.
- Brand fidelity: are the customer's real colors, font, and terminology actually used (not placeholder blue / "Customer")? Does it look like the inspiration tool from Phase 1, or like a generic form?
- Density vs. the Phase 1 audience: is there enough on the page for the named personas, without being cluttered for execs?

**Fix every ❌ yourself before involving the user.** Don't ask the user to approve a page you already know is below the bar — close your own gaps first, redeploy, then grill.

**Step 4.5B — Grill the user (pointed approve/iterate questions, not "looks good?").**

Once your self-critique passes, take a screenshot if you can, then ask the user to react to specifics. Vague "does this look okay?" gets a reflexive "yep" and ships a mediocre demo. Ask sharp, opinionated questions and make it easy to push back:

- "Does this feel like **<inspiration tool>** or like a Salesforce form? Be honest — if it's the latter, tell me what's missing."
- "Top to bottom: the **<hero metric>** number, the **<chart>**, the **<table>** — is the *most important thing for <demo-room persona>* the first thing they see? If not, what should lead?"
- "Brand check: are these the right colors and is **<term>** the word they actually use for **<entity>**?"
- "What's the ONE thing on this page that would make **<the wow-moment persona>** lean in — and is it prominent enough?"
- "Anything here that looks fake, placeholder, or 'demo-y' in a bad way?"

Iterate on their answers until they're genuinely impressed — not just compliant. Then fire the nudge/notification and get explicit sign-off ("yes, ship it") before moving to Phase 5.

> 🔔 Treat this as a real wait point — **fire a desktop notification** (see "Nudge the user") and put the grill-me questions as the LAST block in your message.
...
| "UIBundle Metadata API is not enabled" (or a domain error) on deploy | React App Domain not enabled in the SDO (Slack prework Step 4 skipped) | Have the user: Setup → search **Multi-Framework** → click **Enable Domain** under "Enable the Salesforce App Domain", then redeploy (Phase 2) |
...
| Claude MCP "Couldn't reach the MCP server" | Wrong URL format for the org type | Use the `/sandbox/platform/sobject-reads` path for an SDO/sandbox |
